...
namespace Metis.Overseer.Controllers {
    ...
    public class LogBookController : ControllerBase {
        ...
        [Route ("members/{bookId}")]
        [HttpGet]
        public async Task<IActionResult> GetBookMembers (string bookId) {
            var book = await _logService.GetBook (bookId);
            var members = book.Members.OrderBy (o => o.Name);
            return Ok (members);
        }
        ...
        public async Task<IActionResult> GetBookEntry (string bookId, string entryId) {
            ...
            if (entryId == "new") {
                ...
                var title = User.Identity.GetTitle ();
                var userId = User.Identity.GetUserId ();
                var bookEntry = new LogEntry ();
                bookEntry.Issuer = new Member {
                    Email = email,
                    UserId = userId,
                    Name = title
                };
                bookEntry.LogBookId = bookId;
                bookEntry.DTG= DateTime.UtcNow;
                return Ok (bookEntry);
            }
